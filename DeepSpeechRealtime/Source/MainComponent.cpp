...
MainComponent::MainComponent()
{
    ...
    if (hot_words)
    {
        ...
        for ( std::string hot_word_ : hot_words_ )
        {
            ...
            if (status != 0 || !boost_is_valid) { /*TODO: Post failure to GUI console */ }
            else { /*TODO: Post success to GUI console */ }
        }        
    }
}
...
ds_audio_buffer MainComponent::GetAudioBuffer(const char* path, int desired_sample_rate)
{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    //TODO: Update tmpnam correctly: https://stackoverflow.com/questions/3299881/tmpnam-warning-saying-it-is-dangerous
    //char* output_name = mkstemp(); //mkstemp returns int not char*
#pragma clang diagnostic pop
    assert(output_name);
    sox_format_t* output = sox_open_write(output_name, &target_signal, &target_encoding, "raw", NULL, NULL);
#else
    sox_format_t* output = sox_open_memstream_write(&res.buffer, &res.buffer_size, &target_signal, &target_encoding, "raw", NULL);
#endif
    ...
    {
        fprintf(stderr, "Warning: original sample rate (%d) is lower than %dkHz. Up-sampling might produce erratic speech recognition.\n", desired_sample_rate, (int)input->signal.rate);
    }
...
void MainComponent::ProcessFile(ModelState* context, const char* path, bool show_times)
{
    ...
    ds_result result = LocalDsSTT(context, (const short*)audio.buffer, audio.buffer_size / 2, extended_metadata, json_output);
    free(audio.buffer);
